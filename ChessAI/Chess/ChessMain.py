    ...
    while running:
        ...
        if not gameOver and not humanTurn:
            ...
            if AIMove is None:
                ...
                AIMove = SmartMoveFinder.findRandomMove(gs, validMoves) # backup Random
...
def highlightSquares(screen,gs,validMoves,sqSelected):
    if sqSelected!=():
        r,c = sqSelected
        if gs.board[r][c][0]==('w' if gs.whiteToMove else 'b'): # đúng lượt
            s=p.Surface((SQ_SIZE,SQ_SIZE))
            s.set_alpha(100) #transparent value ->0:transparent

            s.fill(p.Color('green')) # highlight ô được chọn
            screen.blit(s,(c*SQ_SIZE,r*SQ_SIZE))

            s.fill(p.Color('blue')) # highlight ô có thể đi
